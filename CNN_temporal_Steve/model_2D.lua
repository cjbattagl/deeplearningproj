...
local nstates = {20,50,500}         -- exp. 2
--local convsize = {3, 11} 			   -- exp. 1
local convsize = {5, 5}            -- exp. 2
local convstep = {1, 1}
local convpad  = {(convsize[1]-1)/2, (convsize[2]-1)/2}
local poolsize = {2, 2} 			-- exp. 1, 2
local poolstep = {2, 2} 			-- exp. 1, 2
...
if opt.model == 'CNN' then
   ...
   CNN:add(nn.SpatialConvolutionMM(1,nstates[1],convsize[1],convsize[1],convstep[1],convstep[1],0,convpad[1])) -- 48*1024
   local sizeConv1_w = (nfeature-convsize[1]+2*convpad[1])/convstep[1]+1
   local sizeConv1_h = (nframe-convsize[1]+2*convpad[1])/convstep[1]+1

   CNN:add(nn.ReLU())
   CNN:add(nn.SpatialMaxPooling(poolsize[1],poolsize[1],poolstep[1],poolstep[1])) -- 24*512
   local sizePool1_w = (sizeConv1_w-poolsize[1])/poolstep[1]+1
   local sizePool1_h = (sizeConv1_h-poolsize[1])/poolstep[1]+1

   --CNN:add(nn.Dropout(opt.dropout)) -- dropout

   -- stage 2: conv -> ReLU -> Pooling   
   CNN:add(nn.SpatialConvolutionMM(nstates[1],nstates[2],convsize[2],convsize[2],convstep[2],convstep[2],0,convpad[2])) -- 24*512
   local sizeConv2_w = (sizePool1_w-convsize[2]+2*convpad[2])/convstep[2]+1
   local sizeConv2_h = (sizePool1_h-convsize[2]+2*convpad[2])/convstep[2]+1

   CNN:add(nn.ReLU()) 
   CNN:add(nn.SpatialMaxPooling(poolsize[2],poolsize[2],poolstep[2],poolstep[2]))  -- 12*256
   local sizePool2_w = (sizeConv2_w-poolsize[2])/poolstep[2]+1
   local sizePool2_h = (sizeConv2_h-poolsize[2])/poolstep[2]+1

   CNN:add(nn.Dropout(opt.dropout)) -- dropout

   -- stage 3: linear -> ReLU -> linear
   --local ninputFC = 50*256*6 -- exp. 1
   local ninputFC = 50*256*9 -- exp. 2
