...
convolutional layers capture patterns in spatial locality, while deep topologies can capture the compositional nature of images.
The problem of \emph{video} classification introduces a new, temporal dimension is not leveraged by standard 2D CNNs. For instance, a video of a human subject may contain temporal concepts such as gestures or gait that cannot be extracted from a single frame. This prompts the question:
...
\subsection{Results}
\begin{table}[h!]
 \caption{Best test results for a given train-test split of UCF101~\cite{ucf101}, with our results followed by results from the literature.}
\centering
 \begin{tabular}{||l l||}
 \hline
 CNN (ResNet) & 74.97\% \\
