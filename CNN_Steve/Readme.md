...
1. similar as "run_UCF101_final.lua", but using the model "ResNet-101"
2. extract the feature before the full-connected layer
3. generate the name list as well
...
1. similar as "run_UCF101_final.lua", but using the caffe model "VGG_M"
2. extract the fc-6 feature (follow the LRCN paper)
3. generate the name list as well
...
1. modelLang:	choose 'torch' or 'caffe'
2. typeCode:	based on different sample codes ==> 1. original; 2. ResNet
...
2. no need to specify the video number ==> use all the videos above the user-defined minimal frame#
3. follow the original author's split sets to generate 3 training/testing sets
...
1. need "classify_video.lua" or "gen_feature.lua" to do classification or feature generation
2. need to download the [NIN](https://www.dropbox.com/s/brisguefxy9wih5/nin_nobn_final.t7?dl=0) model.
